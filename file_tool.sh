...
while true; do
    echo "Enter a command: create, copy, combine, delete, or exit."
    read -r -p "Enter a command: " cmd
    case "$cmd" in
        create) read -r -p "Filename: " file; read -r -p "Text: " text; create_file "$file" "$text" ;;
        copy) read -r -p "Source file: " src; read -r -p "Destination file: " dest; copy_file "$src" "$dest" ;;
        combine) read -r -p "First file: " f1; read -r -p "Second file: " f2; read -r -p "Output file: " out; combine_files "$f1" "$f2" "$out" ;;
        delete) read -r -p "File to delete: " file; delete_file "$file" ;;
        exit) break ;;
        *) echo "Invalid command." ;;
    esac
done
